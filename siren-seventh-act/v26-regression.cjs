...
T.set({difficulty:'normal',actions:Array.from({length:8},(_,i)=>`action-${i}`),clues:[],flags:{degraded:{},connectionAttempts:{},connectionLocks:{},clueVersion:0}});const firstShift=T.queueProgress();assert.equal(firstShift.id,'machineWipe','normal progress triggers the first concealment event');assert.ok(T.get().clues.includes('freshBrakeOil'),'an attempted mechanical cleanup leaves useful new evidence');delete T.get().flags.pendingProgressEvent;T.get().actions=Array.from({length:18},(_,i)=>`action-${i}`);const secondShift=T.queueProgress();assert.equal(secondShift.id,'editingErase','later progress triggers the editing-room cover-up');assert.ok(T.get().clues.includes('deletionIndex'),'the deletion attempt leaves a recoverable index');delete T.get().flags.pendingProgressEvent;T.get().actions=Array.from({length:30},(_,i)=>`action-${i}`);const thirdShift=T.queueProgress();assert.equal(thirdShift.id,'costumeMove','advanced progress triggers the costume evidence move');assert.ok(T.get().clues.includes('gloveAconite'),'moving the costume evidence exposes another useful trace');assert.equal(T.get().flags.progressEvents.length,3,'each normal progress event occurs only once');
T.set({difficulty:'hard',actions:Array.from({length:40},(_,i)=>`action-${i}`),clues:[],flags:{degraded:{},connectionAttempts:{},connectionLocks:{},clueVersion:0}});assert.equal(T.queueProgress(),null,'progress compensation events stay exclusive to normal mode');
