...
source += `\n;globalThis.T={
  ...
  confrontReady(){return confrontationReady();},confrontRequirements(){return confrontationRequirements();},completeConfront(){completeConfrontation();return state;},metrics(){return endingMetrics();},
...
T.set({clues:['channels','overrideBurn','springMissing','maintenanceLog','freshBrakeOil'],connections:['victimAlive'],interviews:{liangyin:{cornered:true},han:{closed:true}},flags:{degraded:{},connectionAttempts:{},connectionLocks:{},clueVersion:5}});assert.equal(T.confrontReady(),false,'final confrontation stays locked without the rescue connection');
...
console.log('v2.4 regression: all assertions passed');
